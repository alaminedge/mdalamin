...
async function ensureTables(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS blog_users (
    ...
    password TEXT NOT NULL, role TEXT DEFAULT 'user', is_blocked INTEGER DEFAULT 0,
    membership_type TEXT DEFAULT 'free', membership_expires DATETIME,
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS categories (
    ...
    description TEXT, show_on_homepage INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS articles (
    ...
    excerpt TEXT, body TEXT NOT NULL, category_id INTEGER, author_id INTEGER,
    featured_image TEXT, is_premium INTEGER DEFAULT 0, status TEXT DEFAULT 'draft',
    scheduled_at DATETIME, published_at DATETIME,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();

  await db.prepare(`CREATE TABLE IF NOT EXISTS article_images (
    id INTEGER PRIMARY KEY AUTOINCREMENT, article_id INTEGER, image_url TEXT NOT NULL,
    uploaded_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS notice_reads (
    id INTEGER PRIMARY KEY AUTOINCREMENT, notice_id INTEGER NOT NULL, user_id INTEGER NOT NULL,
    read_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(notice_id, user_id))`).run();

  await db.prepare(`CREATE TABLE IF NOT EXISTS homepage_featured (
    id INTEGER PRIMARY KEY AUTOINCREMENT, article_id INTEGER UNIQUE, sort_order INTEGER DEFAULT 0)`).run();
}
...
async function handleAuth(method, path, body, db) {
  if (method === 'POST' && path === '/signup') {
    ...
    try {
      const result = await db.prepare('INSERT INTO blog_users (name,email,password) VALUES (?,?,?)')
        .bind(name, email.toLowerCase(), hashed).run();
      const user = await db.prepare('SELECT * FROM blog_users WHERE id=?').bind(result.meta.last_row_id).first();
      const token = await signToken({ id: user.id, email: user.email, role: user.role, exp: Date.now() + 30*24*60*60*1000 });
      return json({ token, user: { id: user.id, name: user.name, email: user.email, role: user.role, membership_type: user.membership_type, is_blocked: user.is_blocked } });
    } catch (e) {
  ...
  if (method === 'POST' && path === '/login') {
    ...
    const hashed = await sha256(password);
    const user = await db.prepare('SELECT * FROM blog_users WHERE email=? AND password=?')
      .bind(email.toLowerCase(), hashed).first();
    if (!user) return err('Invalid email or password', 401);
    if (user.is_blocked) return err('Account is blocked', 403);
    const token = await signToken({ id: user.id, email: user.email, role: user.role, exp: Date.now() + 30*24*60*60*1000 });
    return json({ token, user: { id: user.id, name: user.name, email: user.email, role: user.role, membership_type: user.membership_type, is_blocked: user.is_blocked } });
  }
...
// ═══ PUBLIC BLOG ═══
async function handleBlog(method, path, body, db, user) {
  // GET /api/blog/categories
  if (method === 'GET' && path === '/categories') {
    const cats = await db.prepare('SELECT * FROM categories WHERE show_on_homepage=1 ORDER BY sort_order ASC').all();
  ...
  if (method === 'GET' && path.match(/^\/article\/[a-z0-9-]+$/)) {
    ...
    const images = await db.prepare('SELECT * FROM article_images WHERE article_id=?').bind(article.id).all();
  ...
  if (method === 'GET' && path === '/notices') {
    const notices = await db.prepare('SELECT * FROM notices WHERE is_active=1 ORDER BY created_at DESC').all();
  ...
  if (method === 'POST' && path.match(/^\/read-notice\/\d+$/)) {
    ...
    await db.prepare('INSERT OR IGNORE INTO notice_reads (notice_id, user_id) VALUES (?,?)').bind(noticeId, user.id).run();
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/categories') {
    ...
    try {
      const result = await db.prepare('INSERT INTO categories (name,slug,description,show_on_homepage) VALUES (?,?,?,?)')
    ...
    } catch(e) { return err(e.message.includes('UNIQUE') ? 'Category slug already exists' : e.message); }
  }
  ...
  if (method === 'POST' && path === '/articles') {
    ...
    if (!title || !articleBody) return err('Title and body required');
    const articleSlug = slug || title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    try {
      const result = await db.prepare(
        `INSERT INTO articles (title,slug,excerpt,body,category_id,author_id,featured_image,is_premium,status,scheduled_at,published_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`
      ).bind(title, articleSlug, excerpt || '', articleBody, category_id || null, user.id, featured_image || null,
             is_premium ? 1 : 0, status || 'draft', scheduled_at || null,
             status === 'published' ? new Date().toISOString() : null).run();
      return json({ id: result.meta.last_row_id, slug: articleSlug, message: 'Article created' });
    } catch(e) { return err(e.message.includes('UNIQUE') ? 'Slug already exists' : e.message); }
  }

  if (method === 'PUT' && path.match(/^\/articles\/\d+$/)) {
    const articleId = parseInt(path.split('/')[2]);
    // Moderator can only edit own articles
  ...
  if (method === 'POST' && path === '/upload-image') {
    ...
    try {
      ...
      if (!imgData.success) return err('Image upload failed');
      const url = imgData.data.url;
      if (article_id) {
        await db.prepare('INSERT INTO article_images (article_id, image_url) VALUES (?,?)').bind(article_id, url).run();
      }
      return json({ url });
  ...
  if (method === 'POST' && path === '/grant-membership') {
    ...
    await db.prepare('UPDATE blog_users SET membership_type = ?, membership_expires = ? WHERE id = ?')
...
export async function onRequest(context) {
  ...
  if (fullPath.startsWith('/blog/')) return handleBlog(request.method, fullPath.replace('/blog',''), body, db, authUser);
